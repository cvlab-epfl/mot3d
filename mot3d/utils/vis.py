        ...
        for j,(track,index,color) in enumerate(zip(tracks, indexes, colors)):
            ...
            if i in index:
                ...
                for t0,t1 in zip(track[t_init:ii], track[t_init+1:ii+1]):
                    img = cv2.line(img, tuple(t0.astype(np.int32)), tuple(t1.astype(np.int32)), color=color, thickness=max(1, int(thickness*0.5)))

                img = cv2.circle(img, tuple(track[ii].astype(np.int32)), radius=thickness, color=color, thickness=-1)
