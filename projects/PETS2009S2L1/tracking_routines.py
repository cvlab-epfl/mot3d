...
def multiview_matching_parallel(detections_2d, views, calibration, max_dist, 
                                distance_none, n_candidates, n_min_views_clique, 
                                verbose):
    ...
    cliques, costs = mm.solve_ilp(g, views, weight_f=lambda d: np.exp(-d**2/max_dist**2), verbose=verbose)
    ...
    for p,clique in zip(positions_3d, cliques):
        if len(clique)>=n_min_views_clique:
            ...
            # remove the 2d detections that ahve been used
            for d in clique:
                detections_2d[d.view].remove(d)        
