...
import mot3d
from mot3d.utils.singleview_geometry import project_points
from multiview_aircraft.utils import utils
import feeders
from multiview_tracking_2d3d import recover_trajectory_2d
            ...
            for k,(traj, indexes, (idx_min, idx_max)) in enumerate(zip(trajectories[class_], 
                ...
                if i>=idx_min and i<=idx_max:
                    ...
                    if len(idxs_to_plot):
                        ...
                        bboxes = []
                        for j in idxs_to_plot:
                            box = getattr(traj[j], 'bbox', None)
                            if box is None and \
                                getattr(traj[j], 'detections_2d', None) is not None and\
                                    view in traj[j].detections_2d:
                                box = getattr(traj[j].detections_2d[view], 'bbox', None)
                            bboxes.append(box)
...
def main(config_file='',
    ...
    __c__ = utils.yaml_read(__c__['configs']['3D'])
    ...
    prefix3d = "_smooth" if use_smoothed else "" + "_2d3d_3D"
    prefix = "_smooth" if use_smoothed else "" + "_2d3d_"+view
    
    basename_config = os.path.splitext(os.path.basename(config_file))[0] 
    
    scene3d = Scene()
    output_file = os.path.join(output_path, "results{}_{}.pickle".format(prefix3d, basename_config))
    print("results scene 3d: {}".format(output_file))
    scene3d.load(output_file)    
    
    scene = Scene2d3d(scene3d) 
    output_file = os.path.join(output_path, "results{}_{}.pickle".format(prefix, basename_config))
    print("results scene 2d: {}".format(output_file))
    ...
    trajs = [recover_trajectory_2d(traj, 
                                   {**scene3d.active, **scene3d.completed}, 
                                   calibration)[0] for traj in trajs]

    mot3d.plot_trajectories(trajs, display_time=True, display_time_every=10)

    plt.savefig(os.path.join(output_path, "plot_trajectories{}_{}.jpg".format(prefix,basename_config)), 
                bbox_inches='tight')              
    ...
    if use_tracklets:
        ...
        output_frames=os.path.join(output_path, "{}{}".format(basename_config, prefix))
        output_video=os.path.join(output_path, "{}{}.mp4".format(basename_config, prefix))          
    ...
    # preparing the mask for displaying where 3d tracking is done
    ground = utils.json_read(__c__['valid_regions'])['ground']
    ground = np.hstack([ground, np.zeros((len(ground),1))])
    ...
    img = imageio.imread(filenames[0])

    proj, mask = project_points(ground, K, R, t, dist, image_shape=img.shape)
    proj = cv2.convexHull(np.float32(proj), returnPoints=True).squeeze()
    ...
    def process_image(image):
        alpha = 0.75
        image_sup = cv2.addWeighted(image, alpha, mask_polygon, 1-alpha, 0) 
        return image_sup
...
# python visualise_2d3d.py -c config/config_2d3d_PETS2009S2L1.yaml -v View_001